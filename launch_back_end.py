...
def create_with_existing_disks(
    project_id: str, zone: str, instance_name: str, disk_names: List[str], ip_adrress: str
) -> compute_v1.Instance:
    """
    ...
    instance = create_instance(project_id, zone, instance_name, attached_disks,external_access=True, external_ipv4 = ip_adrress)
    return instance

project_id = 'cloud1-mse'
zone = 'europe-west6-a'
instance_name = 'backend-todo-1'
ip_adrress = '34.65.168.20'
global backend_ip

create_disk_from_snapshot(project_id,zone,'backend-todo-disk','zones/europe-west6-a/diskTypes/pd-balanced',10,'projects/cloud1-mse/global/snapshots/snapshot-backend')

instance_backend = create_with_existing_disks(project_id,zone, instance_name, ['backend-todo-disk'], ip_adrress)
...
while request is not None:
    ...
    for instance in response['items']:
        ...
        if(instance['name'] == instance_name):
            global backend_ip
            arg = "compute instances add-tags " + instance_name + " --zone " + zone + " --tags " + "ipv4-http"
            #subprocess.run(["c:/system32/gloud.cmd", arg])
